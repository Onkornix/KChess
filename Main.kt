...
fun printBoard(){
    ...
    fun icon(piece: Piece, type: String) : String{ //player color
        ...
        return when(type){
            "pawn" -> "${pColor()}♟$resetColor"
            "bishop" -> "${pColor()}♝$resetColor"
            "king" -> "${pColor()}♚$resetColor"
            "queen" -> "${pColor()}♛$resetColor"
            "rook" -> "${pColor()}♜$resetColor"
            "knight" -> "${pColor()}♞$resetColor"
            else -> "N"
        }
    }
